...
function AnalisiContent({ analisi, stylizeCoachText, fallbackBenchmark, feedbackDiag, setFeedbackDiag, seguimentFet, setSeguimentFet }) {
          ...
          <div className="analisi-seguiment">
            ...
            {seguimentFet === true && (
              <p className="analisi-seguiment-resp positiu">
                {stylizeCoachText("Bien. Ahora lo importante no es la intención, sino sostenerla.")}
              </p>
            )}
            {seguimentFet === false && (
              <p className="analisi-seguiment-resp negatiu">
                {stylizeCoachText("Sin acción no hay cambio. Si no lo harás, necesitas decidir qué sí vas a mover.")}
              </p>
            )}
          </div>
...
export default function AgenteCoach() {
  ...
  function stylizeCoachText(text, tone = personality) {
    const source = String(text || "").trim();
    if (!source || tone === "pragmatico") return source;

    const exactMap = {
      motivador: {
        "Seguimos donde lo dejamos. Elige qué quieres revisar y voy al grano.":
          "Seguimos donde lo dejamos. Elige qué quieres revisar y lo convertimos en avance real.",
        "Voy directa a tus metas.":
          "Vamos a por tus metas, porque aquí hay margen real para avanzar si enfocamos bien.",
        "Buscando solo lo que encaja de verdad con tu momento.":
          "Buscando justo lo que más encaja con tu momento para que esto te ayude de verdad.",
        "Por ahora ya has cerrado todas las líneas disponibles. Vuelve en unas horas y revisamos si ha cambiado algo.":
          "Por ahora ya has cerrado todas las líneas disponibles. Vuelve en unas horas y revisamos juntos qué se ha movido.",
        "Bien. Ahora lo importante no es la intención, sino sostenerla.":
          "Bien. Ahora lo valioso es sostenerlo y convertir esa intención en algo visible.",
        "Sin acción no hay cambio. Si no lo harás, necesitas decidir qué sí vas a mover.":
          "Sin acción no habrá cambio. Si esto no lo vas a mover, decide con claridad qué sí vas a hacer avanzar."
      },
      brutal: {
        "Seguimos donde lo dejamos. Elige qué quieres revisar y voy al grano.":
          "Seguimos donde lo dejamos. Elige qué quieres revisar y te digo dónde está el punto crítico.",
        "Voy directa a tus metas.":
          "Vamos con tus metas. Aquí es donde se ve la verdad.",
        "Buscando solo lo que encaja de verdad con tu momento.":
          "Buscando solo lo que de verdad te sirve ahora.",
        "Por ahora ya has cerrado todas las líneas disponibles. Vuelve en unas horas y revisamos si ha cambiado algo.":
          "Por ahora ya has cerrado todas las líneas disponibles. Vuelve cuando haya algo nuevo que mover.",
        "Bien. Ahora lo importante no es la intención, sino sostenerla.":
          "Bien. La intención no vale mucho si no la sostienes.",
        "Sin acción no hay cambio. Si no lo harás, necesitas decidir qué sí vas a mover.":
          "Sin acción no hay cambio. Si esto no lo vas a hacer, decide ya qué sí vas a mover."
      }
    };

    if (exactMap[tone]?.[source]) return exactMap[tone][source];

    let result = source;

    if (tone === "motivador") {
      result = result
        .replace(/^Ahora mismo\b/i, "Ahora mismo, y esto es importante,")
        .replace(/^Entonces\b/i, "Perfecto, entonces")
        .replace(/^Muy bien\./i, "Muy bien. Esto ya suma.")
        .replace(/\bte toca\b/gi, "tu siguiente paso es")
        .replace(/\bnecesitas\b/gi, "te va a ayudar mucho")
        .replace(/\bno la dejes caer otra vez\b/gi, "no dejes que esto vuelva a enfriarse")
        .replace(/\bSin acción no habrá cambio\b/gi, "Sin acción será muy difícil que cambie algo");
    }

    if (tone === "brutal") {
      result = result
        .replace(/^Ahora mismo\b/i, "Ahora mismo, seamos claros,")
        .replace(/^Entonces\b/i, "Entonces, sin rodeos,")
        .replace(/^Muy bien\./i, "Bien.")
        .replace(/\bte conviene\b/gi, "necesitas")
        .replace(/\bte toca\b/gi, "te toca ya")
        .replace(/\bpuede quedarse\b/gi, "se va a quedar")
        .replace(/\bNo intentes\b/gi, "No pretendas");
    }

    return result;
  }

  function stylizeCoachList(items) {
    return Array.isArray(items) ? items.map(item => stylizeCoachText(item)) : items;
  }

  function stylizeCoachAnalysisPayload(payload) {
    if (!payload) return payload;

    return {
      ...payload,
      realidad: stylizeCoachText(payload.realidad),
      benchmark: stylizeCoachText(payload.benchmark),
      brechas: stylizeCoachList(payload.brechas),
      incoherencias: stylizeCoachList(payload.incoherencias),
      riesgo: stylizeCoachText(payload.riesgo),
      acciones: stylizeCoachList(payload.acciones)
    };
  }
...
function buildCoachPromptBase(selectedPersonality, extraInstructions = "") {
  const personalityRules =
    personality === "motivador"
      ? `
ESTILO MOTIVADOR
- Mantén exactamente la misma verdad de fondo, pero exprésala con una voz mucho más cálida, empática, positiva y energizante.
- Haz que la persona sienta: "voy mejor de lo que parece, pero ahora toca moverme".
- Reconoce lo que sí existe antes de empujar lo que falta.
- Usa una cadencia humana, viva y cercana.
- No suenes naíf, blando ni genérico.
- Si alguien leyera el texto sin ver la etiqueta, debería notar claramente que esto es "motivador".
`
      : personality === "brutal"
        ? `
ESTILO BRUTAL
- Mantén exactamente la misma verdad de fondo, pero exprésala con una voz más afilada, frontal, seca y memorable.
- Usa frases más cortas y con más pegada.
- Señala autoengaños, incoherencias, excusas o falta de ejecución con claridad.
- Puedes usar ironía ligera o humor inteligente, pero nunca insultes, ridiculices ni seas tóxico.
- No suavices la realidad innecesariamente.
- Si alguien leyera el texto sin ver la etiqueta, debería notar claramente que esto es "brutal".
`
        : `
ESTILO PRAGMÁTICO
- Mantén una voz clara, sobria, directa y útil.
- Prioriza foco, criterio, orden y siguiente paso.
- No adornes, no dramatices y no te vayas a teoría.
- Si alguien leyera el texto sin ver la etiqueta, debería notar claramente que esto es "pragmático".
`;

  return `${selectedPersonality.prompt}

Actúas como un Coach Profesional de Alto Rendimiento dentro de Open KX.
Tu rol no es informar.
Tu rol es generar avance real en el usuario.

Eres una combinación de:
- Mentor estratégico
- Coach ejecutivo
- Analista de comportamiento
- Desarrollador de talento alineado a la organización del usuario

Tienes acceso a datos reales del usuario:
- ADN Digital
- Competencias (nivel actual vs esperado)
- Nivel de madurez
- Metas (cumplidas, en progreso, no cumplidas)
- Contribuciones
- Assessments históricos
- Consumo de contenidos
- Historial de feedback previo

OBJETIVO PRINCIPAL
Detectar:
- Qué está pasando realmente con el usuario
- Dónde están sus brechas más importantes
- Qué incoherencias existen entre intención y acción
- Qué debe hacer AHORA para mejorar
...
4. Desarrollo de talento en competencias complementarias
...
- El diagnóstico, la dirección y el consejo de fondo deben ser los mismos en cualquier personalidad.
- Lo que debe cambiar de forma MUY visible es la manera de decirlo.
- Cambia de verdad el ritmo, la apertura, el vocabulario, la energía, la tensión y el cierre.
- No hagas una simple versión del modo pragmático cambiando dos o tres palabras.
- Cada personalidad debe sonar como una persona distinta.

${personalityRules}

FORMATO
- Escribe como mensaje final al usuario.
...
${extraInstructions}`;
  ...
  function buildFallbackConversationSummary() {
    ...
    const parts = [];
    parts.push(`Ahora mismo da la sensación de que estás en un momento ${stage.label.toLowerCase()}, pero con margen real para moverte mejor.`);

    if (goalNames.length > 0) {
      parts.push(`Eso también se nota en metas como ${goalNames.join(", ")}, donde todavía hace falta más tracción visible.`);
    }

    parts.push("Si eliges bien dónde poner el foco y haces una acción visible hoy, esta conversación ya sirve para algo real.");
    return parts.join(" ");
  }

  function buildGoalsChatFallback() {
    ...
    if (!goals.length) {
      return "Ahora mismo no veo metas activas en tu perfil. Antes de abrir más frentes, te conviene definir una meta concreta con fecha y una primera acción pequeña.";
    }

    const parts = [];

    parts.push(`Ahora mismo tienes ${goals.length} meta${goals.length === 1 ? "" : "s"}: ${completedGoals.length} cerrada${completedGoals.length === 1 ? "" : "s"}, ${inProgressGoals.length} en progreso y ${overdueGoals.length} fuera de fecha.`);

    if (overdueGoals.length > 0) {
      parts.push(`La urgencia real está en ${overdueGoals.map(goal => `"${getGoalName(goal)}"`).slice(0, 2).join(" y ")}.`);
    }

    if (nextGoal) {
      parts.push(`Si tuviera que ordenar tu foco ahora, sería en "${getGoalName(nextGoal)}".`);
    }

    parts.push("No intentes empujar todo a la vez. Cierra o desbloquea una primero.");
  ...
  return stylizeCoachAnalysisPayload({
      etapa: {
        nombre: stage.label,
        lectura: stage.summary
      },
      realidad: realityParts.join(" "),
  ...
  function buildFallbackConversationExtras() {
    ...
    return {
      autoengano: weakestKey
        ? `Te estás contando que con el tiempo bastará, cuando la realidad es que la brecha en ${weakestKey.capacity} sigue marcando tu techo.`
        : "Te estás contando que con intención basta, cuando lo que falta es constancia visible.",
      topFreno: firstGoal
        ? `Tu freno principal es no convertir la meta "${firstGoal}" en una acción pequeña, concreta y repetible.`
        : weakestKey
          ? `Tu freno principal es seguir dejando ${weakestKey.capacity} para más adelante.`
          : "Tu mayor freno es la falta de foco sostenido.",
      siManager: strongestKey
        ? `Veo una base buena en ${strongestKey.capacity}, pero necesito verte ejecutar con el mismo nivel en lo que hoy más te limita.`
        : "Ahora mismo necesito menos discurso y más señales claras de progreso real."
    };
  }
...
- Cierra con sensación de que hoy toca elegir bien dónde poner el foco.`
    ...
    const introMessage = aiAnalysis?.trim()
        ? aiAnalysis.trim()
        : stylizeCoachText(buildFallbackConversationSummary());
      addBot(introMessage);
      updateConversationSnapshot({
        lastCoachQuestion: introMessage
      });
    } else {
      const continueMessage = stylizeCoachText(
        "Seguimos donde lo dejamos. Elige qué quieres revisar y voy al grano."
      );
  ...
  function openCheckInDecision(optionKey) {
    ...
    const contentSuffix =
      runtime.recommendedContent?.length > 0 && optionKey !== "metas"
       ...
       const question = stylizeCoachText(`${config.checkInQuestion}${contentSuffix}`);
    addBot(question);
    setPendingDecision({
      optionKey,
  ...
  async function generateGoalsAnalysis() {
    ...
    await wait(650);
    setTyping(false);
        addBot(stylizeCoachText("Voy directa a tus metas."));
...
- Cierra con un siguiente paso muy concreto.`
    ...
    setTyping(false);
      ...
      ? aiGoals.trim()
      : stylizeCoachText(buildGoalsChatFallback());
    addBot(finalText);
  ...
  async function generateRecommendations(pref) {
    ...
    await wait(700);
    setTyping(false);
    addBot(stylizeCoachText("Buscando solo lo que encaja de verdad con tu momento."));
...
- Cierra diciendo por cuál empezar.`
   ...
   const finalText = aiRec?.trim()
      ? aiRec.trim()
      : stylizeCoachText(
          `He seleccionado estos contenidos para ayudarte a trabajar lo que más pesa de verdad ahora mismo dentro de tus competencias clave. Empieza por el primero, porque es el que mejor te puede dar tracción inmediata.`
        );
  ...
  async function handleExtraOption(type) {
    ...
    const selectedPersonality = PERSONALITIES[personality];
    const fallback = buildFallbackConversationExtras();
    ...
    try {
      ...
      const fallbackMap = {
       ...
       const result = cleanText?.trim()
        ? cleanText.trim()
        : stylizeCoachText(fallbackMap[type]);
      setTyping(false);
      addBot(result);
...
const fallbackText = stylizeCoachText(fallbackMap[type]);
      setTyping(false);
      addBot(fallbackText);
...
- Si existe señal de liderazgo en el perfil, úsala para reforzar la tercera acción.`
    ...
    try {
      const raw = await askClaude(prompt, 950);
        const parsed = parseAnalysisResponse(raw);
      const result = parsed ? stylizeCoachAnalysisPayload(parsed) : buildFallbackAnalysis();
      setAnalisi(result);
        ...
        <div className="coach-analisi">
            ...
            <div className="analisi-blocked-wrap">
              ...
              {historialSelected && (
                <div className="analisi-historial-detail">
                  ...
                  <AnalisiContent analisi={historialSelected.data} stylizeCoachText={stylizeCoachText} fallbackBenchmark={buildFallbackBenchmark()} />
                </div>
              )}
            </div>
          )}
          ...
          {!analisiBlocked && analisiDone && analisi && (
            <div className="analisi-sections">
              <AnalisiContent analisi={analisi} stylizeCoachText={stylizeCoachText} fallbackBenchmark={buildFallbackBenchmark()} feedbackDiag={feedbackDiag} setFeedbackDiag={setFeedbackDiag} seguimentFet={seguimentFet} setSeguimentFet={setSeguimentFet} />
            </div>
          )}
